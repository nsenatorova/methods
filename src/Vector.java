...
public class Vector {
    ...
    Vector addV(Vector another){ return new Vector(x + another.x, y + another.y); }
    Vector substractV(Vector another){ return new Vector(x - another.x, y - another.y); }
    Vector multiplyV(double a){ return new Vector(x*a, y*a); }
    ...
    double skalmul(Vector another){ return (x*another.x + y*another.y); }
    double getNorm(){ return Math.sqrt(x*x +y*y); }
    String repr(){ return "(" + x + ", " + y + ")"; }
}
